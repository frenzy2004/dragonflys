...
def main():
    ...
    test_endpoint("GET", "/locations/Cyberjaya/coordinates")
    test_endpoint("GET", "/locations/Kuala Lumpur/coordinates")
    ...
    if result and result.get("success"):
        ...
        for img_type, base64_data in images.items():
            if base64_data:
                save_base64_image(base64_data, f"api_test_images/cyberjaya_{img_type}.png")
    ...
    for img_type in ['before', 'after', 'mask', 'overlay']:
        image_data = test_endpoint(
            "GET",
            f"/detect-change/Cyberjaya/images/{img_type}",
        ...
        if image_data:
            # Save the image
            os.makedirs("api_test_images", exist_ok=True)
            with open(f"api_test_images/cyberjaya_{img_type}_direct.png", "wb") as f:
                f.write(image_data)
            print(f"Saved direct image: cyberjaya_{img_type}_direct.png")
