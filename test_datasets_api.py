...
def test_enhanced_change_detection():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data is None:
                print("❌ Enhanced change detection error: API returned None data")
                return False

            # Check if the analysis was successful
            if not data.get('success', True):
                print(f"❌ Enhanced change detection failed: {data.get('message', 'Unknown error')}")
                return False
            ...
            if socio_data:
                print(f"   💰 Median Income: RM{socio_data.get('median_income', 'N/A'):,}")
                print(f"   📈 Population: {socio_data.get('population', 'N/A'):,}")

            if real_estate_data:
                print(f"   🏠 Avg Home Price: RM{real_estate_data.get('avg_home_price', 'N/A'):,}")
